  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 16, marginBottom: 12, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}><div style={{ width: 14, height: 10, borderRadius: 2, background: C.charcoalLight }} /><span style={{ fontFamily: f, fontSize: 10, color: C.charcoalLight }}>Est. Attendance (K)</span></div>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}><div style={{ width: 14, height: 2, background: C.gold }} /><span style={{ fontFamily: f, fontSize: 10, color: C.charcoalLight }}>Avg Ticket Price ($)</span></div>
      </div>
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", maxWidth: W }}>
        {/* Grid lines */}
        {[0, 25, 50, 75, 100].map(v => <g key={v}><line x1={pad.l} y1={yAtt(v)} x2={W - pad.r} y2={yAtt(v)} stroke={C.creamDark} strokeWidth="0.5" /><text x={pad.l - 8} y={yAtt(v) + 3} textAnchor="end" fontFamily={f} fontSize="9" fill={C.charcoalLight}>{v}K</text></g>)}
        ...
        {/* X axis */}
        {data.map((d, i) => <text key={i} x={x(i)} y={H - pad.b + 18} textAnchor="middle" fontFamily={f} fontSize="9" fill={C.charcoalLight}>{d.yr}</text>)}
        <line x1={pad.l} y1={H - pad.b} x2={W - pad.r} y2={H - pad.b} stroke={C.creamDark} strokeWidth="0.5" />
      </svg>
      <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: 12.5, color: C.charcoalLight, marginTop: 8, lineHeight: 1.5 }}>Pebble Beach Concours, The Quail, and Legends of the Autobahn have seen consistent attendance growth, rising ticket prices, and sellout timelines that compress each year. The economic impact exceeds hundreds of millions annually. 2020 dip reflects COVID cancellations.</p>
    </div>
  );
}
...
function AssetChart() {
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 16, marginBottom: 12, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}><div style={{ width: 14, height: 3, borderRadius: 1, background: C.gold }} /><span style={{ fontFamily: f, fontSize: 10, color: C.charcoalLight }}>Collector Cars (Hagerty Index)</span></div>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}><div style={{ width: 14, height: 2, borderRadius: 1, background: C.charcoalLight }} /><span style={{ fontFamily: f, fontSize: 10, color: C.charcoalLight }}>S&P 500</span></div>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}><div style={{ width: 14, height: 2, borderRadius: 1, background: C.creamDark }} /><span style={{ fontFamily: f, fontSize: 10, color: C.charcoalLight }}>US Real Estate</span></div>
      </div>
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", maxWidth: W }}>
        {[100, 200, 300, 400].map(v => <g key={v}><line x1={pad.l} y1={y(v)} x2={W - pad.r} y2={y(v)} stroke={C.creamDark} strokeWidth="0.5" /><text x={pad.l - 8} y={y(v) + 3} textAnchor="end" fontFamily={f} fontSize="9" fill={C.charcoalLight}>{v}</text></g>)}
        ...
        {years.map((yr, i) => <text key={i} x={x(i)} y={H - pad.b + 18} textAnchor="middle" fontFamily={f} fontSize="9" fill={C.charcoalLight}>{yr}</text>)}
        <line x1={pad.l} y1={H - pad.b} x2={W - pad.r} y2={H - pad.b} stroke={C.creamDark} strokeWidth="0.5" />
        <text x={pad.l} y={H - 4} fontFamily={f} fontSize="8" fill={C.charcoalLight}>Indexed: 2010 = 100</text>
      </svg>
      <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: 12.5, color: C.charcoalLight, marginTop: 8, lineHeight: 1.5 }}>Collector cars have outpaced both the S&P 500 and US real estate over the last 15 years. Total auction volume from RM Sotheby's, Mecum, Gooding, and Bonhams reached record highs through 2024–2026, with individual Ferrari, Porsche, and Mercedes lots setting new benchmarks.</p>
    </div>
  );
}
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "flex-end", height: 210, padding: "0 0 36px" }}>
        {data.map((d, i) => (
          <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
            ...
            <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: C.creamDark, marginTop: 4 }}>{d.label}</span>
          </div>
        ))}
      </div>
      <div style={{ display: "flex", gap: 20, justifyContent: "center" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}><div style={{ width: 12, height: 12, borderRadius: 2, background: C.greenAccent }} /><span style={{ fontFamily: "'DM Mono', monospace", fontSize: 10, color: C.creamDark }}>Revenue</span></div>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}><div style={{ width: 12, height: 12, borderRadius: 2, background: C.gold }} /><span style={{ fontFamily: "'DM Mono', monospace", fontSize: 10, color: C.creamDark }}>Operating Profit</span></div>
      </div>
    </div>
  );
}
...
/* ===== MAIN DECK (public /deck — Pitch Deck v3) ===== */
export default function PitchDeckSlash({
  ...
  return (
    <div style={{ background: C.cream, minHeight: "100vh" }}>
      ...
      <section style={sec(C.cream)}><div style={inner}>
        <FadeIn><SectionTag number={4} label="The Problem" light /><div style={{ ...h2, marginBottom: 8 }}>EVERY COLLECTOR HAS THE SAME TWO PRIORITIES</div><p style={{ ...acc, marginBottom: 28 }}>Whether you own one car or ten, it always comes back to the same two things: managing the ownership and actually enjoying the car.</p></FadeIn>
          ...
          <div style={{ background: C.greenDeep, borderRadius: 10, padding: 28, border: `1px solid ${C.greenMid}44` }}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 16, marginBottom: 16, flexWrap: "wrap" }}>
              ...
              <div style={{ padding: "6px 14px", display: "flex", alignItems: "center" }}>
                <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: 12.5, color: C.creamDark, fontStyle: "italic" }}>all ad-heavy, visually dated, not built for local events</span>
              </div>
            </div>
              ...
              <div style={{ flex: 1, minWidth: 320, background: C.greenPrimary, borderRadius: 12, overflow: "hidden", border: `1px solid ${C.greenAccent}44`, display: "flex", flexDirection: "column" }}>
                ...
                <div style={{ margin: "0 26px 20px", background: C.greenDeep, borderRadius: 8, padding: "16px 18px", border: `1px solid ${C.greenMid}33` }}>
                  ...
                  <div style={{ display: "flex", gap: 20, marginTop: 10 }}>
                    <div><div style={{ ...mono, fontSize: 8, color: C.creamDark }}>VALUE</div><div style={{ fontFamily: "'Saira Extra Condensed', sans-serif", fontWeight: 800, fontStyle: "italic", fontSize: 22, color: C.gold }}>$128,500</div></div>
                    <div><div style={{ ...mono, fontSize: 8, color: C.creamDark }}>TREND</div><div style={{ fontFamily: "'Saira Extra Condensed', sans-serif", fontWeight: 800, fontStyle: "italic", fontSize: 22, color: C.goldLight }}>+12.4%</div></div>
                    <div><div style={{ ...mono, fontSize: 8, color: C.creamDark }}>NEXT SERVICE</div><div style={{ fontFamily: "'Saira Extra Condensed', sans-serif", fontWeight: 800, fontStyle: "italic", fontSize: 22, color: C.creamDark }}>$4,200</div></div>
                  </div>
                </div>
              </div>
              ...
              <div style={{ flex: 1, minWidth: 320, background: C.greenPrimary, borderRadius: 12, overflow: "hidden", border: `1px solid ${C.gold}33`, display: "flex", flexDirection: "column" }}>
                ...
                <div style={{ margin: "0 26px 20px", display: "flex", gap: 8 }}>
                  ...
                  <div style={{ flex: 1, padding: "10px 12px", background: C.greenDeep, borderRadius: 6, textAlign: "center" }}>
                    ...
                    <div style={{ ...mono, fontSize: 8, color: C.creamDark }}>LEGACY FORUMS</div>
                  </div>
                  <div style={{ flex: 1, padding: "10px 12px", background: C.greenDeep, borderRadius: 6, textAlign: "center" }}>
